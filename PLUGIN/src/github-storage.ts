...
export async function savePlan(input: unknown): Promise<SavePlanResult> {
  ...
  // El SHA del espejo se captura antes de escribir el plan canónico. Si otro
  // chat actualiza current.json durante el proceso, no se fuerza el overwrite.
  ...
  try {
    ...
    // El plan canónico ya está seguro; un conflicto del espejo no invalida la publicación.
  }
