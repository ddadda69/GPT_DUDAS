...
Esta skill pertenece únicamente al proyecto independiente `PLUGIN/`.
...
- El almacenamiento del plugin vive exclusivamente en `PLUGIN/data/`.
...
5. El servidor valida y publica primero `PLUGIN/data/plans/<id>.json` y usa `PLUGIN/data/current.json` solo como espejo.
...
- `PLUGIN/data/current.json` es no canónico: un conflicto en el espejo no invalida el archivo del plan.
